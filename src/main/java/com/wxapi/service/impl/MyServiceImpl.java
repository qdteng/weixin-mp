...
public class MyServiceImpl implements MyService{
	Logger log = Logger.getLogger(MyServiceImpl.class);
	@Autowired
	...
	public String processMsg(MsgRequest msgRequest,MpAccount mpAccount){
		...
		log.info(String.format("接收到消息 : %s ", JSONObject.fromObject(msgRequest).toString()));
		...
		log.info(String.format("返回消息 : %s ", respXml));
		return respXml;
	}
	...
	private String processTextMsg(MsgRequest msgRequest,MpAccount mpAccount){
		...
		if(!StringUtils.isEmpty(content)){//文本消息
			...
			MsgText  msgText= msgBaseDao.getMsgTextByInputCode(tmpContent);
			
			if(null!=msgText&&StringUtils.isNotBlank(msgText.getContent())){
